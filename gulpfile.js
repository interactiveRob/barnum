...
const webpackConfig = {
   ...
   output: {
       filename: `${aliases.basename}.${aliases.suffix}.${version}.js`,
...
function svgs() {
    return gulp
        ...
        .pipe(svgmin( { plugins: [{ name: 'removeUselessStrokeAndFill', active: false }] } ))
...
function manifest(cb) {
    $manifest = {
        js: `${aliases.suffix}.${aliases.suffix}.${version}.js`,
...
function watch(cb) {
    ...
    gulp.watch([`${baseDir}/**/*.php`, `${baseDir}/**/*.html`, `${paths.dist}/*.${aliases.suffix}.js`]).on('change', browserSyncReload);
    cb();
}
